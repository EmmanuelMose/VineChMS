...
const sendWithRetry = async (ai, contents, maxRetries = 3) => {
    let lastError;
...
const handleChat = async (req, res) => {
    try {
        ...
        if (!reply || !reply.trim()) {
            return res.status(500).json({
                error: "The VineChMS Assistant could not generate a response. Please try again.",
            });
        }
        ...
        if (message.includes("503") ||
            ...
            return res.status(503).json({
                error: "The VineChMS Assistant is temporarily busy. Please try again in a moment.",
            });
        }
        if (message.includes("429") ||
            message.includes("resource exhausted")) {
            return res.status(429).json({
                error: "The VineChMS Assistant is temporarily receiving too many requests. Please try again shortly.",
            });
        }
        return res.status(500).json({
            error: "The VineChMS Assistant is temporarily unavailable. Please try again.",
        });
    }
};
