...
import { FiUpload, FiX, FiImage, FiMaximize2 } from "react-icons/fi";
...
export default function ChurchProfile({ churchId }: ChurchProfileProps) {
  ...
  const handleLogoUpload = async (file: File) => {
    ...
    try {
      const result = await uploadFileToCloudinary(file, token, "vinechms/churches", {
        resourceType: "image",
        quality: 80,
        width: 400,
        height: 400,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || "Failed to update church profile");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="church-profile">
      <div className="church-profile-header">
        <div className="church-profile-title-section">
          ...
          <p>Update your church information and logo</p>
        </div>
      </div>

      <div className="church-profile-logo-section">
        <div className="church-profile-logo-wrapper">
          {formData.logo ? (
            <div 
              className="church-profile-logo-image clickable"
              onClick={() => setIsImageModalOpen(true)}
            >
              <img src={formData.logo} alt="Church logo" />
              <button
                type="button"
                className="church-profile-remove-logo"
                onClick={(e) => {
                  e.stopPropagation();
                  removeLogo();
                }}
              ...
              <div className="church-profile-logo-expand">
                <FiMaximize2 size={16} />
              </div>
            </div>
          ) : (
            <div className="church-profile-logo-placeholder">
              <FiImage size={32} />
              <span>{formData.name.charAt(0).toUpperCase()}</span>
            </div>
          )}
        </div>
        <div className="church-profile-upload-actions">
          <input
            type="file"
            accept="image/*"
            ref={fileInputRef}
            ...
            className="church-profile-file-input"
            id="church-logo"
          />
          <label htmlFor="church-logo" className="church-profile-file-label">
            <FiUpload size={16} />
          ...
          {formData.logo && (
            <button
              type="button"
              className="church-profile-remove-btn"
              onClick={removeLogo}
            >
              Remove Logo
            </button>
      ...
      {isImageModalOpen && (
        <div className="church-profile-image-modal" onClick={() => setIsImageModalOpen(false)}>
          <div className="church-profile-image-modal-content" onClick={(e) => e.stopPropagation()}>
            <button className="church-profile-image-modal-close" onClick={() => setIsImageModalOpen(false)}>
              <FiX size={24} />
            </button>
            <img src={formData.logo} alt="Church logo" />
          </div>
