// File: frontend/src/pages/dashboard/churchadmindashboard/pledges/PayPledge.tsx
...
import "./PayPledge.css";

interface PayPledgeProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  pledge: Pledge;
...
export default function PayPledge({ 
  ...
  const selectedMember = members.find((m) => m.memberId === pledge.memberId);
  ...
  return (
    <div className="pledges-modal-overlay" onClick={onClose}>
      ...
      <div className="pledges-modal pledges-pay-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="pledges-pay-summary">
          <div className="pledges-pay-member">
            <strong>{selectedMember?.fullName || "Unknown Member"}</strong>
            <span>{selectedMember?.email || ""}</span>
          </div>
          <div className="pledges-pay-amounts">
            <div className="pledges-pay-amount-item">
              <label>Pledged</label>
              <span className="pledges-pay-total">KES {totalAmount.toFixed(2)}</span>
            </div>
            <div className="pledges-pay-amount-item">
              <label>Paid</label>
              <span className="pledges-pay-paid">KES {paidAmount.toFixed(2)}</span>
            </div>
            <div className="pledges-pay-amount-item">
              <label>Remaining</label>
              <span className="pledges-pay-remaining">KES {remaining.toFixed(2)}</span>
            </div>
          </div>
          <div className="pledges-pay-progress">
            <div className="pledges-pay-progress-bar">
              <div 
                className="pledges-pay-progress-fill" 
                style={{ width: `${totalAmount > 0 ? Math.min((paidAmount / totalAmount) * 100, 100) : 0}%` }}
              ></div>
            </div>
            <span className="pledges-pay-progress-text">
              {totalAmount > 0 ? Math.round((paidAmount / totalAmount) * 100) : 0}%
            </span>
          </div>
        </div>

        <div className="pledges-pay-mode-selector">
          <button 
            className={`pledges-pay-mode-btn ${mode === "mpesa" ? "active" : ""}`}
            onClick={() => setMode("mpesa")}
          >
            <FiSend size={16} /> M-Pesa
          </button>
          <button 
            className={`pledges-pay-mode-btn ${mode === "cash" ? "active" : ""}`}
        ...
        <form onSubmit={handleSubmit} className="pledges-modal-form">
          ...
          {!isMpesaMode && (
            <div className="pledges-form-group">
              ...
              <div className="pledges-file-upload-wrapper">
                <input
                  type="file"
                  accept="image/*"
                  ref={fileInputRef}
                  ...
                  className="pledges-file-input"
                  id="pledge-payment-upload"
                />
                <label htmlFor="pledge-payment-upload" className="pledges-file-label">
                  <FiUpload size={16} />
                  {uploadingFile ? "Uploading..." : "Upload Evidence"}
                </label>
              </div>
              {formData.receiptFile && (
                <div className="pledges-file-preview">
                  <span>Evidence uploaded</span>
                  <button 
                    type="button" 
                    onClick={() => openEvidenceModal(formData.receiptFile)}
                    className="pledges-view-evidence"
                  >
                    <FiEye size={16} /> View
                  </button>
                  <button type="button" onClick={removeFile} className="pledges-remove-file">
                    <FiX size={16} />
                  </button>
                </div>
              )}
          ...
          <div className="pledges-modal-actions">
            ...
            <button
              type="submit"
              className="pledges-modal-submit"
              disabled={
                loading ||
                uploadingFile ||
                !formData.amount ||
